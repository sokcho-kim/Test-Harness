...
| 페이지 | 경로 | 기능 |
|--------|------|------|
| Dashboard | `/` | 통계 대시보드, 빠른 실행 |
| Prompts | `/prompts` | 프롬프트 목록/상세/버전 관리 |
| Datasets | `/datasets` | 데이터셋 목록/상세/케이스 관리 |
| Tests | `/tests` | 테스트 목록/생성 위자드/결과 |
| Compare | `/compare` | 모델/프롬프트 비교 분석 |
...
2. **실시간 테스트 진행률**
   - 2초 간격 폴링으로 progress 업데이트
   - 상태별 뱃지 (pending/running/completed/failed)

3. **결과 비교 분석**
   - 모델별 Pass Rate / Latency 비교
   - 프롬프트별 성능 비교
   - Side-by-side 결과 보기
...
| 항목 | ID | 설명 |
|------|-----|------|
| 프롬프트 | `prompt_4236de90` | Medical NER Extraction |
| 데이터셋 | `dataset_f9c1a3aa` | Medical NER Test Dataset (77건) |

### 프롬프트 내용

```
다음 의료 텍스트에서 엔티티를 추출해주세요.
...
- **위치**: `scripts/import_ner_data.py`
- **기능**: 프롬프트 생성 → 데이터셋 생성 → 케이스 임포트
...
2. Tests → New Test 클릭
3. Medical NER Extraction 프롬프트 선택
4. Medical NER Test Dataset (77건) 선택
5. 모델 선택 후 실행
